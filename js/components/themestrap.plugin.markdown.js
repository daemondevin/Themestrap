 ...
 * 
 * SYNTAX HIGHLIGHTING  (PluginHighlight integration)
 *
 *   When options.highlight is true (default) and PluginHighlight is loaded on
 *   the page, every fenced code block rendered by the plugin is automatically
 *   passed to $.fn.themestrapPluginHighlight().  The language is read from the
 *   class="language-*" attribute that the GFM renderer emits — no extra markup
 *   is needed.
 *
 *   If PluginHighlight is not loaded the option is silently ignored and code
 *   blocks retain their plain <pre><code> markup.
 *
 *   Options forwarded to PluginHighlight can be set via options.highlightOptions:
 *
 *     $('#readme').themestrapPluginMarkdown({
 *         highlight: true,
 *         highlightOptions: { lineNumbers: true, showCopy: true }
 *     });
 *
 *   Per-block overrides via data-plugin-options on the rendered <pre> are NOT
 *   supported (the <pre> is generated by the GFM renderer, not authored by the
 *   user).  Use highlightOptions for page-wide defaults.
 *
 *   Highlight instances are tracked in this._highlightInstances and are
 *   destroyed alongside the Markdown instance when destroy() is called.
 *
...
`;
    ...
    class PluginMarkdown {
        ...
        /**
         * Pass every fenced-code <pre> emitted by the GFM renderer to
         * PluginHighlight (if loaded).  Called automatically by render().
         *
         * The GFM renderer writes:
         *   <pre><code class="language-js">…</code></pre>
         *
         * PluginHighlight._normalizePre() walks from <code> up to the nearest
         * <pre>, so targeting the <code> elements directly is safe and avoids
         * any ambiguity with plain <pre> blocks that have no language class.
         *
         * Each highlight instance is stored in this._highlightInstances so that
         * destroy() can tear them down cleanly.
         *
         * @returns {PluginMarkdown}
         */
        _highlightBlocks() {
            if (!this.options.highlight) return this;
            if (!$.isFunction($.fn.themestrapPluginHighlight)) return this;
 
            // Tear down any highlight instances from the previous render pass
            // before creating new ones (re-render scenario).
            this._destroyHighlightInstances();
 
            const self   = this;
            const hlOpts = $.extend(
                { lineNumbers: false, showCopy: true },
                self.options.highlightOptions || {}
            );
 
            // Find every <code class="language-*"> the GFM renderer produced.
            // Blocks without a language class are plain indented-code blocks —
            // leave them unstyled so they don't get unwanted line numbers.
            self.$el.find('code[class*="language-"]').each(function () {
                const $code = $(this);
                const $pre  = $code.closest('pre');
                if (!$pre.length) return; // orphaned <code> — skip
 
                // Extract the bare language string from the class, e.g.
                ...
                // PluginHighlight.setOptions() resolves the language by reading
                // the data-plugin-highlight attribute directly from the <pre>.
                // It does NOT read opts.language from the JS options bag —
                // that path is overridden by the attribute resolution earlier
                // in setOptions().  Setting the attribute here is the only
                // reliable way to pass the language to the plugin.
                $pre.attr('data-plugin-highlight', lang);
 
                // Forward shared highlightOptions.  Remove any language key the
                // caller may have set — the attribute above is authoritative.
                const blockOpts = $.extend({}, hlOpts);
                delete blockOpts.language;
 
                // Call on the <pre> directly — no normalization walk needed.
                $pre.themestrapPluginHighlight(blockOpts);
 
                // Store the instance for clean teardown on destroy().
                const hlInst = $pre.data('__highlight');
                if (hlInst) self._highlightInstances.push(hlInst);
            });
        ...
        /**
         * Destroy all PluginHighlight instances created by _highlightBlocks().
         * @returns {PluginMarkdown}
         */
        _destroyHighlightInstances() {
            this._highlightInstances.forEach(inst => {
                try { inst.destroy(); } catch (_) { /* already torn down */ }
            });
            this._highlightInstances = [];
            return this;
        }
    ...
    PluginMarkdown.defaults = {
        ...
        /**
         * Pass rendered fenced code blocks to PluginHighlight automatically.
         * Requires $.fn.themestrapPluginHighlight to be present on the page.
         * Set to false to disable syntax highlighting entirely.
         */
        highlight: true,
 
        /**
         * Options forwarded to every PluginHighlight instance created for
         * code blocks inside this Markdown element.  These mirror the
         * PluginHighlight.defaults keys — see themestrap.plugin.highlight.js.
         *
         * Defaults applied here:
         *   lineNumbers: false   (line numbers off by default in prose contexts)
         *   showCopy:    true    (copy button retained)
         *
         * Example:
         *   highlightOptions: { lineNumbers: true, showCopy: false }
         */
        highlightOptions: {}
    };
