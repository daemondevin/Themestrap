...
export default function (hljs) {
    ...
    const JSON_NUMBER = {
        scope: 'ts-json.number',
        match: /-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?/,
        relevance: 0,
    };
    const JSON_LITERAL = {
        scope: 'ts-json.literal',
        match: /\b(?:true|false|null)\b/,
    };
    const JSON_PUNCT = {
        scope: 'ts-json.punctuation',
        match: /[{}\[\],:]/,
    };

    const JSON_STRING = {
        scope: 'ts-json.string',
        begin: /"/,
        end: /"/,
        contains: [],
    };
    ...
    const JSON_EMBEDDED = {
        ...
        contains: [JSON_KEY, JSON_STRING, JSON_NUMBER, JSON_LITERAL, JSON_PUNCT],
        relevance: 2,
    };
